...
GEMINI_API_KEY = 'AIzaSyAjrsaNocSpZJfRqEimys6m6Y01_cdCeXs'
...
def retrieve_memory(user_id, emotion, user_input):
    ...
    relevant_memories = []
    print('memories:',all_stored_memories)
...
def chat_with_user(user_id, user_input, happy_memory_provided=False):
    ...
    if emotion_value in ["sadness", "anger", "pessimism", "fear", "disgust"]:
        ...
        print(f'Retrieved memories:{memory}')
        prompt = f"The user is feeling {emotion_value}. Respond with a comforting message and share this memory: '{memory}'. Tell that the memory hopefully makes them feel better and say goodbye."
        response = model.generate_content(prompt)
